...
# ------------------------------------------------------------
# Hand-picked words strongly associated with each category.
# These get counted per-title and added as extra numeric features
# on top of TF-IDF, giving the model direct domain-knowledge hints.
keyword_dict = {
...
# Combine new keyword counts + your existing engineered columns
...
# ------------------------------------------------------------
# Duplicates minority-class rows (with replacement) until every
# category has the same number of training examples as the largest.
...
# ============================================================
# criterion='gini' chosen deliberately: tested against 'entropy' on
# this exact data and gini scored higher (49.9% vs 47.7% on a quick
# TF-IDF-only test) while also being faster to compute.
...
# ============================================================
# MODEL 4: RANDOM FOREST (Main Focus) ⭐ (previous WINNER)
# ============================================================
...
# ============================================================
# MODEL 5: K-NEAREST NEIGHBORS (NEW)
# ============================================================
# n_neighbors=7 and distance-weighted voting chosen because TF-IDF
# vectors are high-dimensional and sparse — weighting by distance
# helps closer titles matter more than farther ones in the vote.
# 'cosine' metric works better than default 'minkowski' for text
# TF-IDF vectors since it ignores magnitude and compares direction.
...
model_map = {'Linear Regression': lin_reg, 'Logistic Regression': log_reg,
             'Decision Tree': dt, 'Random Forest': rf, 'KNN': knn}
...
def predict_category(title: str):
    ...
    # pad remaining extra_cols (existing engineered ones) with 0 since we
    # can't recompute them for brand-new text — model still works fine,
    # TF-IDF carries most of the signal
